...
private registrationService = inject(RegistrationService)
  ...
  title: string = 'Lista trenerów';
  pdfTitle: string = '';
  service = inject(RegistrationService)
  private generalSettingsService = inject(GeneralSettingsService);
  loading = false;
  private snackbar = inject(SnackbarService);
  displayedColumns: string[] = ['rowNumber', 'clubName', 'lastname', 'firstname', 'supper_fri', 'night_fri_sat', 'dinner_sat', 'supper_sat', 'night_sat_sun', 'dinner_sun', 'price_pln'];
  ...
  ngAfterViewInit() {
    ...
    this.dataSource.sortingDataAccessor = (row: any, column: string) => {
      switch (column) {
        ...
        case 'gender': return this.getRoom(row.playerSpec?.personSpec?.gender ?? '').toLowerCase();
     ...
     this.dataSource.filterPredicate = (row: any, filter: string) => {
      ...
      const flat = [
        ...
        p.lastname, p.firstname,
         this.getRoom(p.gender),                 // pokój as text
  ...
  ngOnInit(): void {
    ...
    // 2) Coaches — independent, with spinner
    this.loading = true;
    this.registrationService.readCoaches()
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          ...
          this.snackbar.showError('Błąd w pobieraniu listy trenerów. ' + msg);
        }
      });
  }
